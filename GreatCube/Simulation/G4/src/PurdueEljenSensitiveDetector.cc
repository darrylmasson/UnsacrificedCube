...
PurdueEljenSensitiveDetector::PurdueEljenSensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("EljenDetectorHitsCollection");
}

PurdueEljenSensitiveDetector::~PurdueEljenSensitiveDetector()
{
}

void PurdueEljenSensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pEljenDetectorHitsCollection = new PurdueEljenDetectorHitsCollection(SensitiveDetectorName, collectionName[0]);
	...
	pHitsCollectionOfThisEvent->AddHitsCollection(iHitsCollectionID, m_pEljenDetectorHitsCollection); 
}

G4bool PurdueEljenSensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *)
{
	...
	PurdueEljenDetectorHit* pHit = new PurdueEljenDetectorHit();
	...
	m_pEljenDetectorHitsCollection->insert(pHit);
...
void PurdueEljenSensitiveDetector::EndOfEvent(G4HCofThisEvent *)
{
//  if (verboseLevel>0)
// { 
