...
PurdueParticleSource::ConfineSourceToVolume(G4String hVolumeList)
{
	...
	for(set<G4String>::iterator pIt = m_hVolumeNames.begin(); pIt != m_hVolumeNames.end(); pIt++)
	{
		...
		if((bMatch = ((G4int)hRequiredVolumeName.last('*') != (G4int)std::string::npos)))
...
PurdueParticleSource::ReadEnergySpectrum()
{
	...
	while(!hIn.eof())
	{
		...
		if(hIn.good())
		{
			...
			dBinWidth = dBinEnergy-dBinEnergy_old;			
			dBinEnergy_old = dBinEnergy;			
			hEnergyBins.push_back(dBinEnergy*dFactor);
			hProbabilities.push_back(dProbability);
			hBinWidths.push_back(dBinWidth);
		}
	}
	...
	//G4double dMin_width = hBinWidths.front();
	//G4double dMax_width = hBinWidths.back();

	G4double* dBinEdges = new G4double[iNbBins];
	for (int i=0; i<iNbBins; i++) {
		dBinEdges[i] =  hEnergyBins.at(i);
	}

	m_hEnergySpectrum.Reset();
	m_hEnergySpectrum.SetBins(iNbBins-1, dBinEdges);
