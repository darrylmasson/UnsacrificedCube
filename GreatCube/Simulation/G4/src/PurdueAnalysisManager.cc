...
#include "PurdueEventData.hh"

#include "PurdueAnalysisManager.hh"

PurdueAnalysisManager::PurdueAnalysisManager(PurduePrimaryGeneratorAction *pPrimaryGeneratorAction) :
...
{
  runTime = new G4Timer();
  m_pEventData = new PurdueEventData();
}

PurdueAnalysisManager::~PurdueAnalysisManager()
{}

void
PurdueAnalysisManager::BeginOfRun(const G4Run *)
{
  ...
  G4cout <<"PurdueAnalysisManager:: Init data tree ..."<<G4endl;
  m_pTree = new TTree("events", "Tree containing event data for Purdue Detector");
...
void
PurdueAnalysisManager::EndOfRun(const G4Run *)
{
  runTime->Stop();
...
void
PurdueAnalysisManager::BeginOfEvent(const G4Event *)
{
...
void
PurdueAnalysisManager::EndOfEvent(const G4Event *pEvent)
{
  _events->cd();
  ...
  PurdueEljenDetectorHitsCollection* pEljenDetectorHitsCollection = 0;
  ...
  if(pHCofThisEvent)
    {
      if(m_iEljenDetectorHitsCollectionID != -1)
	{
	  pEljenDetectorHitsCollection = (PurdueEljenDetectorHitsCollection *)(pHCofThisEvent->GetHC(m_iEljenDetectorHitsCollectionID));
  ...
  if(iNbEljenDetectorHits)
	...
	{
	  PurdueEljenDetectorHit *pHit = (*pEljenDetectorHitsCollection)[i];
...
void PurdueAnalysisManager::Step(const G4Step *)
{
}
