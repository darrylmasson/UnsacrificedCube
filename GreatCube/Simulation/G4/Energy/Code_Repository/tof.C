...
int main(int argc, char **argv)
{
  ...
  double angle(40.);
  double seperation(1000.);
  double detector1(200.);
  string filename;
  ...
  string outputpath = "~/JP_Purdue/Energy/prodata/";
  ...
  */
  ...
  events->SetBranchAddress("type", &particle_type);

  //Set the ranges in which to look for events 
  cout<<"I think detector1 is at "<<detector1<<endl;
  cout<<"I think the angle is "<<angle<<endl;
  cout<<"I think the seperation is "<<seperation<<endl;
  double y1min, y1max, y2min, y2max =0;
  y1min = detector1 - 40.;
  y1max = detector1 + 40.;
  y2min = y1min + seperation*cos(angle*PI/180);
  y2max = y1max + seperation*cos(angle*PI/180);
  cout<<"I am looking for events in range "<<y2min<<" to "<<y2max<<endl;
  ...
  */
  ...
  for (int i=0; i < entries; ++i)
      ...
      for (int k=0; k<vectorsize; k++)
	{
	  ...
	  if (yposition->at(k)>y2min && yposition->at(k)<y2max && process->at(k)!="Transportation" && particle_type->at(k) == "neutron")
	    {
	      if (xposition->at(k)>0)
		{
		  ...
		  for (int m=0; m<vectorsize; m++)
		    {
		      ...
		      if (yposition->at(m)>y1min && yposition->at(m)<y1max && process->at(k)!="Transportation" && particle_type->at(k) == "neutron")
			{
  ...
  cout<<"There are "<<eventsofinterest<<" possible doublescatters"<<endl;
  cout<<"Hits in 2 :"<<hitsintwo<<endl;
  cout<<"Hits in 3 :"<<hitsinthree<<endl;
  /*
